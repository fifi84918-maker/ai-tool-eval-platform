...
class TestSkillsAPI:
    ...
    def test_search_skills_pagination(self, client):
    ...
    def test_get_skill_nonexistent_returns_404(self, client):
        """GET /api/v1/skills/nonexistent returns 404."""
        response = client.get("/api/v1/skills/nonexistent-skill-id")
        assert response.status_code == 404
        assert "not found" in response.json()["detail"].lower()

    def test_openapi_json_returns_200_with_paths(self, client):
    ...
    def test_no_sensitive_data_leak_in_responses(self, client):
