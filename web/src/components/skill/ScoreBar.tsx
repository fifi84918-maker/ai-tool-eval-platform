...
export default function ScoreBar({ score }: ScoreBarProps) {
  if (score === null || score === undefined) {
    return (
      <div className="flex items-center gap-2">
        <div className="flex-1 bg-[#E5E6EB] rounded-full h-2">
          <div className="bg-gray-400 h-2 rounded-full" style={{ width: '0%' }} />
        </div>
        <span className="text-sm text-[#86909C] w-12 text-right">—</span>
      </div>
    )
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm text-[#1D2129] font-medium w-12 text-right">
        {score.toFixed(1)}
      </span>
    </div>
  )
